# Week 10：一般化線形モデル：ロジスティック回帰分析 {.unnumbered}
...
- 線形回帰を2値のデータに当てはめたモデルは、本来は1（合格）もしくは0（不合格）しかとらないはずだが、予測値（直線）が1以上であったり0以下になっている。
...
- 切片の値はマイナスとなっており、0-1という確率の範囲を超えている。
  ...
  - よりよいモデルでは、予測値が0から1に収まることが期待される。
...
::: callout-note
**オッズ**とはある事象が「起こる確率」を「起こらない確率」で割った値。例えば、ドジャースが勝つ確率が75%であれば、負ける確率は25%（100% - 75%）。つまり、勝つオッズは3:1。
**オッズ比**とは、一方の群のオッズを、もう一方の群のオッズで割った値。2群で事象の起こりやすさが等しい場合には、オッズ比は1、ある群（分子の群）における事象の起こりやすさが別の群（分母の群）よりも大きい場合は、1より大きくなる。例えば、ドジャースが勝つ確率が75%（オッズは3:1）、ホワイトソックスが勝つ確率が10%（オッズは1:9）である場合、ドジャースの勝利オッズをホワイトソックスの勝利オッズで割ったオッズ比は、

$$
\begin{aligned}
&=
\frac{\text{ドジャースが勝つオッズ}}
{\text{ホワイトソックスが勝つオッズ}} \\[8pt]
&=
\frac{0.75 / (1 - 0.75)}
{0.10 / (1 - 0.10)} \\[8pt]
&=
...
つまり、この例では、ドジャースが勝つオッズは、ホワイトソックスが勝つオッズの27倍。

<u>注意点として、競馬などのかけ事のオッズと統計学のオッズは意味合いが異なる。例えば、JRAの定義するオッズは、[「勝馬投票券が的中した場合の概算払戻率」](https://www.jra.go.jp/kouza/yougo/w406.html)を指す。</u>

:::

- 確率（ *P* ）を対数オッズ（オッズに対数を取ったもの）に変換。ロジット変換という。
...
- ロジット関数により、
  ...
  - 従属変数の値が0か1かという制約がなくなり、線形モデルとして偏回帰係数を推定できる。
    
    - <u>線形モデルを当てはめる方が計算などの都合がいい。</u>

- 注意点として、**線**形モデルとは異なり、**曲線**であるため、ロジットスケールでは同じ差でも、元の確率スケールでは場所によって意味が（1単位の変化量が）異なる。

  - logit(0.5) = 0、logit(0.6) ≈ 0.4 
  
    - ロジットスケールでの0.4の変換は変換前の単位での50%から60%の変更に対応
  
  - logit(0.9) ≈ 2.2、logit(0.93) ≈ 2.6
  
    - ロジットスケールでの0.4の変換は変換前の単位での90%から93%の変更に対応
...
logit_data <- data.frame(
  Before_Logit = p_values,
  After_Logit = logit_values
)
# 注目する点
point_data <- data.frame(
  x = c(0.50, 0.60, 0.90, 0.93),
  y = qlogis(c(0.50, 0.60, 0.90, 0.93)),
  label = c("50%", "60%", "90%", "93%")
)
# 矢印で示す区間
arrow_data <- data.frame(
  x_start = c(0.50, 0.90),
  x_end   = c(0.60, 0.93),
  y       = c(-3.2, 1.8),   # 矢印を置く高さ（見やすいように調整）
  label   = c("0.50 → 0.60", "0.90 → 0.93")
)

ggplot(logit_data, aes(x = Before_Logit, y = After_Logit)) +
  geom_line(color = "blue", linewidth = 1) +
  
  geom_vline(xintercept = c(0.50, 0.60),
             linetype = "dashed", color = "red") +
  geom_vline(xintercept = c(0.90, 0.93),
             linetype = "dashed", color = "darkgreen") +
  
  geom_point(data = point_data, aes(x = x, y = y), size = 2) +
...
## ロジスティック関数
- この関数を使うことで、説明変数の集まり（線形予測子：線形結合した説明変数）がどの範囲に合っても、0 ~ 1の範囲に収まる。つまり、確率を表すのに最適！。

  - ロジスティック回帰ではリンク関数にロジット関数をおき、確率を線形予測子に変換

  - ロジスティック関数は逆リンク関数として使われ、線形予測子から確率を復元する
  
    - リンク関数：従属変数を変換し、独立変数関数につなげる変換関数。
...
- 一般線形モデルのように、回帰係数をそのまま解釈できない
  ...
  - 変換係数を経由して、説明変数の変化が影響を受けている
    ...
    - 線形モデル：ある独立変数Aが1単位変化すると、従属変数はBだけ変化
...
::: callout-important
```glm```では、**Estimate**は従属変数そのものの尺度での変化量ではなく、リンク関数で変換した後の尺度での係数である。例えばロジスティック回帰では、目的変数は0/1のデータであるが、Estimateは確率そのものの変化量ではない。Estimateはロジット、つまり対数オッズの変化量を表す。よって、Estimateが1増えるということは、「確率が1%増える」という意味ではなく、「対数オッズが1増える」という意味である。
:::

- **Std. Error (Standard Error):** 標準誤差。推定値の不確実性を表す。どれくらい推定に誤差があるかを示す指標。「同じ調査法で同じ数のデータをとりなおしてみると、推定値も結構変わるので、そのバラツキ度合い」

- ***z* value：**z値と呼ばれる統計量（Wald統計量とも言われる）。Estimate ÷ SE で算出。推定値がゼロから標準誤差何個分離れているかを表す。絶対値が大きいほど、「係数がゼロ」という帰無仮説から離れていると考えられる。

- **Pr(>|z|):** z値に対応する、Wald検定の両側　*p*　値。帰無仮説として「係数が0である」と仮定したときに、今回得られたz値と同じか、それ以上に極端な値が得られる確率。

- ***p* 値（アスタリスク）：***p*値の大きさを記号であらわしている。Wald型の95%信頼区間に0を含まない場合、対応するWald検定ではおおむね *p* < .05となる。
...
### 作図
- ```predict```関数に、```type = "response"```を指定することで、確率の単位が確認できる。
...
### 結果報告の例と解釈
- 英語の学習年数の主効果の係数は有意ではなかった（*Estimate* = -0.21 [-0.47, 0.03], *SE* = 0.12, *z* = -1.67, *p* = .10, *OR* = 0.81 [0.63, 1.03]）。傾向として、英語学習開始歴が1年増えると、テストに合格する**オッズ**が0.81倍になる（ = 合格するオッズが [1 - 0.81 = 0.19] 19%低い）と予測される。

- もし係数が0.21だった場合、オッズ比は```exp(0.21) = 1.23```。この場合、

  - 英語学習歴が1年増えると、テストに合格するオッズが1.23倍になる。つまり、テストに合格するオッズは（不合格となる場合に比べ）（1.23 - 1 = 0.23）23%増加すると予測される。
...
# 各年数に対する予測確率を計算
pred_probs <- predict(res, newdata = data.frame(Starting_age = years_range), 
                      type = "response") #"response"にすると、予測確率が返される
...
- 💻[オッズ比1](https://bellcurve.jp/statistics/course/26726.html)
