...
- 同じことを何度も繰り返したらどうなるかについての**仮想的**な分布
...
integrate(
  f = dnorm, 
  lower = mean(density_data$x) - 1.96, 
  upper = mean(density_data$x) + 1.96
  )
```
...
- 母平均が分からない場合、上記の*t*値を計算することはできない。しかし、母平均の信頼区間を求める場合、*t*分布の95%の面積にあるような範囲を考えればいい。自由度25の時、上側と下側それぞれ2.5%（5% ÷ 2）となるような*t*値を求める（表から値を探したりして値を見つける）。
...
- $u^2$ は不偏分散であり、以下の式で求められる。自由度25、標準偏差が2.24の場合、*n*に25、*S*に2.24を代入する。

  - 標本サイズが大きいときはほとんど同じ値になるため、実用上標本分散を用いることもある。
...
$$
  ...
  - 上記の計算から、98.52 - 100.40の間に母平均があると推測される。
...
- 「仮に母集団からの無作為抽出と区間の算出という操作を**無限回繰り返したとした場合に**、そのようにして算出された区間のうち、95%の割合で真の母数の値を含むことになる」ように構成された区間
  ...
  - 「母数がその区間に含まれる確率が95%」という解釈は間違い
    ...
    - 実際にデータをとって計算した場合は、その95%信頼区間に母数を含むか、含まないかしか分からない

      - 信頼区間の計算式の確率変数が実数値に代わる
...
::: callout-important
まとめると、標準誤差は標本分布の広がりを表し、広がりの度合いをもとに（95%）信頼区間が計算される。
:::
...
::: callout-important
qmdファイルのYAMLヘッダーには以下のコードを加えてください。HTMLファイルを作成した際に、作図したデータをHTMLファイルの中に埋め込むので、フォルダーが作られるのを防ぎます。
```self-contained: true```
:::
