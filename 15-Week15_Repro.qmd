...
-   [Language LearningのHPより（2026年7月24日現在）](https://onlinelibrary.wiley.com/page/journal/14679922/homepage/ForAuthors.html#badgel)

> **Open Science Initiatives** <mark style="background-color: orange;">Language Learning encourages authors to share the data, materials, research instruments, and other artefacts supporting the results in their study by archiving them in an appropriate public repository.</mark> Examples of qualifying public, open-access databases of data, research instruments, and materials are the IRIS digital repository, the Open Science Framework repository, and the SLA Speech Tools repository. Authors who archive the data, materials, research instruments, testing protocols, and/or other research artefacts used in their study should include an accessibility statement with a link to the repository they have used.

## 1. Quartoを使って分析結果をまとめることができる
...
- Quartoでは様々な形式に出力可能！[参考](https://quarto.org/docs/output-formats/all-formats.html)
...
- テンプレートを追加することも可能。formatに```reference-doc```を指定する。テンプレートはきちんとレベル分けの設定などを行っておく必要あり。
...
-   無料で利用できるが、無料版の場合、リンクを知る人だれもが閲覧できるので注意

### Githubを使う

-   Githubとは、コードのバージョン管理をするツール。共同編集可能。
-   今回は一番簡単な方法で実行します。しかし、Git(hub)をフルで使いこなせば、ファイルのバージョン管理、共同編集などが可能でより再現可能な資料作成に一歩近づくと思います。
...
- 寺井のおすすめ。最初に作成した日付と、アップデートした日付を表示します。
...
date: 
  value: now
  format: "[Published: 2026-07-27, last update:] YYYY-MM-DD HH:mm:ss"
```
...
-   Quartoファイルの冒頭にこのようにまとめてある。確かに、その分析で使うパッケージが一目瞭然だが、個別の処理においてどのパッケージが読み込まれたのか分かりにくく、そのコードを参考に書き換えにくくなる。
...
- 基本的にRのようにコードで命令する必要があるが、Positronでは様々な機能がクリック操作で実行可能。

## まとめ
<div style="border: 3px solid blue; padding: 10px; background-color: #e6f7ff;">
1.  Quartoでは、YAMLヘッダー、コードチャンク、ドキュメントチャンクを使って分析結果をまとめることができる。

    -   研究ごとにプロジェクトを作成しておくとよい

2.  作成したQuartoファイルは、Word、HTMLファイル、Power Pointなど様々な形式に出力できる。

    -   Githubなどを使えば、ウェブページとして共有できる
