...
- **相関**：2つのデータの間にある**線形**の関係の強さを示す。これを分析することを**相関分析**という
  ...
  - 相関係数は2変数の関係が線形になっていない場合、その関係性を適切に評価できない。

    - 下記の図の右では明確にU字型の関係が確認できるが、相関係数はかなり小さい値となっている。
...
# 曲線描画用データ
new_x <- seq(min(x2), max(x2), length.out = 200)
pred_y <- predict(
  fit2,
  newdata = data.frame(x2 = new_x)
)
...
::: callout-note
- 相関係数を算出する際によく使われるRパッケージの```cor```は計算する際に、共分散ではなく、**不偏**共分散を使用している（下記は```help(cor)```の結果）。

> The denominator *n* − 1 is used which gives an unbiased estimator of the (co)variance for i.i.d. observations. 

- 不偏共分散とは以下の式で定義されるように、分母を -1 調整した共分散である（不偏推定量）。分母の値（標本サイズ）を1減らすことで、減らさない場合に比べ、値が大きくなる。標本から計算した（共）分散は一致性を持つが、不偏性を持たないとされている。そのため分母を調整した不偏（共）分散が使用されている。
...
- 検定統計量は *t* 分布に従うことを利用して検定が行われる。
  ...
  - 効果の大きさとサンプルの大きさの積で計算されていることが分かる。
$$
...
::: callout-note  
無相関検定で有意差が得られても、母相関係数が0ではないということを示すだけであり、相関の**強弱を示していない**ことに注意が必要。
:::
...
- 相関係数が小さい研究は査読の段階で削除されていたり、そもそも出版されずお蔵入りになっている場合もあるので、それらを含めると上記の値よりも小さくなる可能性もあることに注意。
...
3. **（再来週までに提出でOK）**現時点での研究計画をまとめる。自分の研究に重要な先行研究でもOK。**2の宿題とは別のHTMLファイルにまとめて提出。**
...
- 📚武藤（2026）『入門これからの心理統計 ー推定と検定の統一的理解をめざしてー』
