...
## 分析可能なデータ（セット）の作成
- 分析用のコードを書く不可は生成AIの登場で各段に下がりました。しかし、分析の前に、分析ができるデータを用意しないといけません。記録した参加者の答えなどをそのまま分析に使用できることはほとんどありません（e.g., 個人情報を変えたり）。つまり、分析のためには、データを分析可能な形に加工するプロセスがあります。このプロセスは生成AIが登場しても今だ手間のかかるプロセスです。
