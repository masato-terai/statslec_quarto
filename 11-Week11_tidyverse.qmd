...
## 分析可能なデータ（セット）の作成
- 分析用のコードを書く負荷は生成AIの登場で各段に下がりました。しかし、分析の前に、分析ができるデータを用意しないといけません。記録した参加者の答えなどをそのまま分析に使用できることはほとんどありません（e.g., 個人情報を変えたり）。つまり、分析のためには、データを分析可能な形に加工するプロセスがあります。このプロセスは生成AIが登場しても今だ手間のかかるプロセスです。

- データを分析可能な形に加工する場合、手動でコピーや削除をする方が大勢います。しかしこれはかなり危険です。なぜならば加工のプロセスは自動的に記録されないからです。また、加工にミスがあってもそれに気づくのがかなり難しいです。

- しかし、これらの処理をプログラミング言語を使って行えば、加工のプロセスがすべてコードとして記録されます。間違った加工をしていても、コードを見ればどこが間違っているのか特定できます。したがって、元データと加工したデータの往復が自由です。「手でやる方が早い！」と思うかもしれませんが、長期的な目で見ればこちらの方が効率が良いです。また、研究の透明性という観点でも当然こちらの方が良いです。
...
- R 内で作成・加工した変数は.csv、.xlsx、.txtの形式でエクスポート可能
...
write.table(
  ...
  "mtcars.txt",      #ファイル名
  sep = "\t",        #区切り文字。カンマなら","
  row.names = FALSE, #データの一列目に行番号を含めるか
  quote = FALSE      #文字列を引用符 " " で囲むか
)
```
...
- ```ggsave()```を使用すると、綺麗に```.png```形式で保存可能

- 事前に作成した図を変数に格納する必要がある。

  - e.g., ```p <- ggplot(mpg, aes(x = class, y = cty)) + geom_boxplot()```

```{r}
#| eval: false

ggsave(
  "R-plot.png", #ファイルの名前を指定
  plot = p,     #エクスポートしたい図を格納している変数名
  dpi = 300,    #解像度の指定。初期値は300
  unit = "cm",  #保存する画像の大きさの単位（縦横の単位を定義）
                #"in", "cm", "mm", "px"
  width = 5,    #横幅
  height = 5,   #縦幅
  bg = "white"  #これを指定しない場合、背景が透明になる。初期値はNULL
)
...
- 💻[ggsave()で高画質かつ高解像度のグラフを保存する](https://say.boo.jp/277/#toc2)