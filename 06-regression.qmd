# Week 6：単回帰分析（連続型の説明変数） {.unnumbered}
...
- 回帰直線の式に上記の式を代入。*x* が平均 $\bar{x}$ に等しいとき、 *y* の予測値 $\hat{y}$ は *y* の平均 $\bar{y}$ に等しくなる。つまり、回帰直線は、点（$\bar{x}$, $\bar{y}$）を通る傾き *b* の直線

  - また、回帰直線が平均を基準にどれだけズレているか、そのズレに応じてyの予測値がどれくらい変化するかを表していることもわかる。
...
- 事前分布を狭く指定しない場合、頻度統計の結果もベイズ統計の結果も近似する。
...
- **残差（residual）**：予測の誤差ともいう。それぞれの観測値と直線の差のこと。つまり、$e = y - \hat{y}$ 。
  ...
  - 予測値 $\hat{y}$ と残差の相関は0
    ...
    - 予測値は変数 *x* を線形変換したもので、相関係数の絶対値は変わらないから
...
3. 現時点での研究計画をまとめる。自分の研究に重要な先行研究でもOK。**2の宿題とは別のHTMLファイルで提出。**
