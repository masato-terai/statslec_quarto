# Week 9：重回帰分析 (3)：交互作用の解釈 {.unnumbered}

```{r}
#| include: false

library(rstan)
library(brms)
library(gt)
library(tidyverse)

rstan_options(auto_write = T)
options(rstan.show_progress = FALSE)
options(mc.cores = parallel::detectCores())
```

## 事前の確認

-   この講義用のフォルダ－📁をPositronで開いていますか？

## 今日の目標

1. 重回帰分析の交互作用項について概念的に理解できる
2. 交互作用を含む重回帰分析を実装しその結果を適切に報告できる

## 交互作用
- 2つ以上の独立変数の組み合わせが従属変数にもたらす影響
  
  - 一般化線形モデルでは、「説明変数同士の積」を説明変数として加えることで、交互作用を表現する

::: callout-caution
以下の交互作用に関する図の結果は、パターンを分かりやすく表示するため、意図的に効果を操作している。よって、実際のデータのパターンとは結果が異なる。
:::

```{r}
#| echo = FALSE
dat <- tribble(
  ~Pattern, ~Type, ~Evolution, ~HP,
  
  # ① タイプ差あり・進化差あり
  "タイプ差あり\n進化差あり", "水", "進化前",  8,
  "タイプ差あり\n進化差あり", "雷", "進化前", 12,
  "タイプ差あり\n進化差あり", "水", "進化後", 12,
  "タイプ差あり\n進化差あり", "雷", "進化後", 16,
  
  # ② タイプ差あり・進化差なし
  "タイプ差あり\n進化差なし", "水", "進化前", 10,
  "タイプ差あり\n進化差なし", "雷", "進化前", 14,
  "タイプ差あり\n進化差なし", "水", "進化後", 10,
  "タイプ差あり\n進化差なし", "雷", "進化後", 14,
  
  # ③ タイプ差なし・進化差あり
  "タイプ差なし\n進化差あり", "水", "進化前", 10,
  "タイプ差なし\n進化差あり", "雷", "進化前", 10,
  "タイプ差なし\n進化差あり", "水", "進化後", 14,
  "タイプ差なし\n進化差あり", "雷", "進化後", 14,
  
  # ④ 主効果なし
  "主効果なし", "水", "進化前", 12,
  "主効果なし", "雷", "進化前", 12,
  "主効果なし", "水", "進化後", 12,
  "主効果なし", "雷", "進化後", 12
)

dat$Pattern <- factor(dat$Pattern,
               levels = c(
                "主効果なし", "タイプ差なし\n進化差あり",
                "タイプ差あり\n進化差なし", "タイプ差あり\n進化差あり"
               ))

ggplot(
  dat,
  aes(
    x = Type,
    y = HP,
    group = Evolution,
    color = Evolution,
    linetype = Evolution
  )
) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  facet_wrap(~ Pattern, nrow = 1) +
  scale_y_continuous(
    limits = c(1, 20),
    breaks = seq(0, 20, 5)
  ) +
  labs(
    x = "ポケモンのタイプ",
    y = "HP得点",
    color = "",
    linetype = ""
  ) +
  theme_bw(base_size = 14) +
  theme(
    legend.position = "top"
  ) +
  ggtitle("交互作用なし")

dat_interaction <- tribble(
  ~Pattern, ~Type, ~Evolution, ~HP,
  
  # ① 交差する
  "交差する", "水", "進化前",  8,
  "交差する", "雷", "進化前", 16,
  "交差する", "水", "進化後", 16,
  "交差する", "雷", "進化後",  8,
  
  # ② 差が広がる
  "差が広がる", "水", "進化前", 10,
  "差が広がる", "雷", "進化前", 12,
  "差が広がる", "水", "進化後", 10,
  "差が広がる", "雷", "進化後", 18,
  
  # ③ 差が縮まる
  "差が縮まる", "水", "進化前",  8,
  "差が縮まる", "雷", "進化前", 16,
  "差が縮まる", "水", "進化後", 12,
  "差が縮まる", "雷", "進化後", 14,
  
  # ④ 片方だけ変化
  "片方だけ変化", "水", "進化前", 10,
  "片方だけ変化", "雷", "進化前", 10,
  "片方だけ変化", "水", "進化後", 10,
  "片方だけ変化", "雷", "進化後", 17
)

ggplot(
  dat_interaction,
  aes(
    x = Type,
    y = HP,
    group = Evolution,
    color = Evolution,
    linetype = Evolution
  )
) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  facet_wrap(~ Pattern, nrow = 1) +
  scale_y_continuous(
    limits = c(1, 20),
    breaks = seq(0, 20, 5)
  ) +
  labs(
    x = "ポケモンのタイプ",
    y = "HP得点",
    color = "",
    linetype = ""
  ) +
  theme_bw(base_size = 14) +
  theme(
    legend.position = "top"
  ) +
  ggtitle("交互作用あり")
```

## 変数のタイプごとの交互作用の解釈

### 量的変数 × 量的変数

```{r}
#| include: false

dat <- read.csv("sample_data/3-10-3-interaction-3.csv")
```

```{r}
#| cache: true

model1 <- brm(sales ~ product * clerk, gaussian(link = "identity"),
              data = dat, seed = 123, refresh = 0, iter = 5000, chains = 2, silent =2)
```

```{r}
#| include: false
#| cache: true

eff <- conditional_effects(model1, effects = "product:clerk")
```

- 緑色（平均値）の線、赤色の線は右肩上がりになっている

```{r}
plot(eff, plot = F)[[1]]
```

```{r}
#| echo: false

sjPlot::tab_model(model1)
```

- 量的 × 量的変数の交互作用の予測値

  - 切片 + Product × (-2.26) + Clerk × (6.49) + **（Product × Clerk） × 1.05**
  
  - 店員数（Clerk）によって、製品数（Product）の係数の値が変化する

```{r}
#| echo: false

new_data <- data.frame(
  product = c(0, 10, 0, 10),
  clerk = c(0, 0, 10, 10)
)

res <- round(fitted(model1, new_data), 2)

cbind(new_data, res) %>%
  gt() %>%
  tab_header(title = "量的 × 量的変数の交互作用") %>%
  cols_align(align = "center") %>%
  tab_options(table.width = pct(100))
```

### 量的変数 × 質的変数
- データの読み込み
```{r}
dat <- read.csv("sample_data/3-10-2-interaction-2.csv")
```

- コーディング（トリートメント）

  - 他のコーディングでも分析可能である
  
```{r}
dat$publicity <- factor(dat$publicity)
contrasts(dat$publicity) <- contr.treatment(2)
contrasts(dat$publicity)
```

```{r}
#| cache: true

model2 <- brm(sales ~ publicity * temperature, gaussian(link = "identity"),
              data = dat, seed = 123, refresh = 0, iter = 5000, chains = 2, silent =2)
```

```{r}
#| include: false
#| cache: true

eff2 <- conditional_effects(model2, effects = "temperature:publicity")
```

- 宣伝アリの方が、切片（線の開始点）も傾きも大きいことが分かる
```{r}
plot(eff2, plot = F)[[1]]
```
```{r}
#| echo: false

sjPlot::tab_model(model2)
```

- 質的 × 量的変数の交互作用の予測値

```{r}
df <- data.frame(
  宣伝 = c("なし", "あり"),
  `売り上げの予測値` = c(
                 "42.80 + temperature × 2.59",
                 "42.80 + 17.49 + temperature × (2.59 + 4.19)")
)

df %>%
  gt() %>%
  tab_header(title = "質的 × 量的変数の交互作用") %>%
  cols_align(align = "center") %>%
  tab_options(table.width = pct(100))

```

- 気温の主効果は、宣伝がなかった時における気温の効果であることに注意。そのため、気温の主効果の数値だけではなく、交互作用を確認して結果を解釈する必要がある。

```{r}
#| echo: false

new_data_2 <- data.frame(
  publicity = rep(c("not", "to_implement"), each = 2),
  temperature = c(0, 10, 0, 10)
)

res2 <- round(fitted(model2, new_data_2), 2)

cbind(new_data_2, res2) %>%
  gt() %>%
  tab_header(title = "質的 × 量的変数の交互作用") %>%
  cols_align(align = "center") %>%
  tab_options(table.width = pct(100))
```

### 質的変数 × 質的変数

```{r}
dat <- read.csv("sample_data/3-10-1-interaction-1.csv")
```

- コーディング（トリートメント）
```{r}
dat$publicity <- factor(dat$publicity)
contrasts(dat$publicity) <- contr.treatment(2)
contrasts(dat$publicity)
```

```{r}
dat$bargen <- factor(dat$bargen)
contrasts(dat$bargen) <- contr.treatment(2)
contrasts(dat$bargen)
```

```{r}
#| cache: true

model3 <- brm(sales ~ publicity * bargen, gaussian(link = "identity"),
              data = dat, seed = 123, refresh = 0, iter = 5000, chains = 2, silent =2)
```


```{r}
#| include: false

eff3 <- conditional_effects(model3, effects = "publicity:bargen")
```

- 宣伝も安売りもありの方が売り上げが最も大きいことが分かる
```{r}
plot(eff3, plot = F)[[1]]
```

```{r}
#| echo: false

sjPlot::tab_model(model3)
```

- 質的 × 質的変数の交互作用の予測値

  - 宣伝も安売りもあった場合、二つの主効果に加え、交互作用の影響が加算される
  
```{r}
#| echo: false

df <- data.frame(
  宣伝 = c("なし", "あり", "なし", "あり"),
  安売り = c("なし", "なし", "あり", "あり"),
  `売り上げの予測値` = c("103.32", "103.32 + 10.05", "103.32 + 27.35", "103.32 + 10.05 + 27.35 + 20.72")
)

df %>%
  gt() %>%
  tab_header(title = "質的 × 質的の交互作用") %>%
  cols_align(align = "center") %>%
  tab_options(table.width = pct(100))

```

- 得られた係数の結果と比較するとさらに分かりやすい
```{r}
#| echo: false

new_data_3 <- data.frame(
  publicity = rep(c("not", "to_implement"), 2),
  bargen = rep(c("not", "to_implement"), each = 2)
)

res3 <- round(fitted(model3, new_data_3), 2)

cbind(new_data_3, res3) %>%
  gt() %>%
  tab_header(title = "質的 × 質的変数の交互作用") %>%
  cols_align(align = "center") %>%
  tab_options(table.width = pct(100))
```

## 下位検定
- 交互作用が有意だった場合に、交互作用の中身を詳しく検討するために単純主効果の検定を行う（検定の多重性に注意）。

  - 単純主効果
  
    - ある要因の各水準における、別の要因の効果のこと
  
    - e.g., Publicity条件における、Bargen実施の有無（to_impement/not）の平均値差

  - 多重比較

    - 単純主効果において比較する1要因の水準が3以上などの場合、単純主効果の検定と合わせて行い、どの水準間に差があるかを調べる必要がある。
  
- 交互作用が有意
```{r}
#| echo: false

model4 <- lm(sales ~ publicity * bargen, data = dat)
sjPlot::tab_model(model4,show.se = T, show.stat = T)
```

```{r}
#| echo: false

ggplot(dat, aes(x = publicity, y = sales, group = bargen, color = bargen)) +
  stat_summary(fun = mean, geom = "line", aes(group = bargen)) + 
  stat_summary(fun = mean, geom = "point", size = 3) +  # 平均値の点
  theme_minimal()
```

- ```emmeans()```関数を使って、下位検定を行う

```{r}
#install.packages("emmeans")
library(emmeans)
```

-  bargenの単純主効果（publicityの各水準ごとにbargenの効果を調べる）

  - bargen by publicity（縦の比較）

  - publicityがnotの場合もto_implementの場合も条件間の差は有意
```{r}
#| cache: true

emmeans::emmeans(
  model4,
  ~ bargen | publicity
) %>%
  emmeans::contrast(
    method = "pairwise",
    adjust = "bonferroni"
  )
```


-  publicityの単純主効果（bargenの各水準ごとにpublicityの効果を調べる）

  - publicity by bargen 

  - bargenがto_implementの場合のみ条件間の差は有意

```{r}
#| cache: true
emmeans::emmeans(
  model4,
  ~ publicity | bargen
) %>%
  emmeans::contrast(
    method = "pairwise",
    adjust = "bonferroni"
  )
```


- 報告の例

宣伝と安売りの有無、そしてそれらの交互作用が売り上げに与える影響を調べるため、回帰分析を行った。宣伝と安売りはトリートメント・コントラストでコーディングし、いずれも「なし」を0、「あり」を1とした。分析の結果、安売りおよび宣伝×安売りの交互作用の係数が有意であった（安売り: *b* = 27.27, 95% CI [17.05, 37.49], *SE* = 5.15, *t* = 5.30, *p* < .001；交互作用: *b* = 20.80, 95% CI [6.34, 35.25], *SE* = 7.28, *t* = 2.86, *p* = .005）。

交互作用が有意であったため、単純主効果の検定を行った。

安売りの単純主効果は、宣伝の有無にかかわらず有意であった（宣伝なし: *b* = -27.3, *SE* = 5.15, *t* = -5.30, *p* < .001；宣伝あり: *b* = -48.1, *SE* = 5.15, *t* = -9.34, *p* < .001）。これにより、宣伝の有無にかかわらず、安売りは売上向上に寄与する可能性が示唆された。

また、安売りがある条件では、宣伝の効果が有意であった（*b* = -30.73, *SE* = 5.15, *t* = -5.97, *p* < .001）。一方、安売りがない条件では、宣伝の効果は有意ではなかった（*b* = -9.94, *SE* = 5.15, *t* = -1.93, *p* = .057）。

## ハンズオンセッション

### 質的 × 質的（サムコントラスト）

```{r}
dat <- read.csv("sample_data/Example4.csv")
```

```{r}
head(dat, 5)
```

```{r}
table(dat$Grade)
```

```{r}
table(dat$Learning)
```

```{r}
dat$Grade <- factor(dat$Grade)
dat$Learning <- factor(dat$Learning, 
  levels = c("List", "Context", "KeyWord") #Wは大文字なので注意
  )
```

- 各群ごとの平均
```{r}
stats::aggregate(Post ~ Grade, data = dat, FUN = mean)
```

```{r}
stats::aggregate(Post ~ Learning, data = dat, FUN = mean)
```

```{r}
res <- stats::aggregate(
  Post ~ Grade * Learning,
  data = dat, 
  FUN = mean)

res
```

- Ground Mean
```{r}
mean(res$Post)
```

```{r}
contrasts(dat$Grade) <- contr.sum(2)
contrasts(dat$Grade)
```

```{r}
contrasts(dat$Learning) <- contr.sum(3)
contrasts(dat$Learning)
```

```{r}
model_sum <- lm(Post ~ Grade * Learning, data = dat)
```

```{r}
summary(model_sum)
```

- 以下のような作図を行う場合、コーディングによって図中のパターンが変わることはない
```{r}
#install.packages("sjPlot")
library(sjPlot)
plot_model(
  model_sum, 
  type = "pred",
  terms = c("Grade", "Learning")
           )
```

### 係数の解釈
- 記述統計の値を参照して考えると分かりやすい
  - **(Intercept):** Grand Mean (47.5)
  - **Grade1:** First - grand mean (39.93333 - 47.5)
  - **Learning1:** List - grand mean (65.1 - 47.5)
  - **Learning2:** Context - grand mean (32.5 - 47.5)
  - **Grade1:Learning1:** 

$$
\frac{
  2M_{\text{リスト1}} - M_{\text{文脈1}}
  - M_{\text{キー1}} - 2M_{\text{リスト2}}
  + M_{\text{文脈2}} + M_{\text{キー2}}
  }
  {6}
$$

$$
\frac{2(M_{\text{リスト1}} - M_{\text{リスト2}}) + (M_{\text{文脈2}} - M_{\text{文脈1}}) + (M_{\text{キー2}} - M_{\text{キー1}})}{6}
$$

$$
\frac{{2(60 - 70.2) + (45.2 - 19.8) + (49.8 - 40)}}{6}
$$

```{r}
(
  (2 * (60 - 70.2)) + 
  (45.2 - 19.8) +
  (49.8 - 40)
) / 
  6
```

::: callout-important
Rでは掛け算・割り算が足し算・引き算より先に計算される。式全体を割りたい場合は括弧 () で囲む必要がある。
:::

  - **Grade1:Learning2:** 

$$
\frac{
  - M_{\text{リスト1}} + 2M_{\text{文脈1}}
  - M_{\text{キー1}} + M_{\text{リスト2}}
  - 2M_{\text{文脈2}} + M_{\text{キー2}}
  }
  {6}
$$

$$
\frac{(M_{\text{リスト2}} - M_{\text{リスト1}}) + 2(M_{\text{文脈1}} - M_{\text{文脈2}}) + (M_{\text{キー2}} - M_{\text{キー1}})}{6}
$$

$$
\frac{{(70.2 - 60) + 2(19.8 - 45.2) + (49.8 - 40)}}{6}
$$

```{r}
(
  (70.2 - 60) + 
  (2 * (19.8 - 45.2)) + 
  (49.8 - 40)
 ) /
  6
```

### 質的 × 質的（反復コントラスト）

```{r}
#install.packages("MASS")
library(MASS)
contrasts(dat$Grade) <- fractions(contr.sdif(2))
contrasts(dat$Grade)
```

```{r}
contrasts(dat$Learning) <- fractions(contr.sdif(3))
contrasts(dat$Learning)
```

```{r}
model_rep <- lm(Post ~ Grade * Learning, data = dat)
```

```{r}
summary(model_rep)
```

### 係数の解釈
- **(Intercept):** grand mean
- **Grade2-1:** Second - First
- **Learning2-1:** Context - List
- **Learning3-2:** Keyword - Context
- **Grade2-1:Learning2-1:**

$$
(M_{\text{リスト1}} + M_{\text{文脈2}}) 
- (M_{\text{文脈1}} + M_{\text{リスト2}})
$$

$$
(M_{\text{文脈2}}-M_{\text{文脈1}}) 
- (M_{\text{リスト2}} - M_{\text{リスト1}})
$$

```{r}
(45.2 - 19.8) - (70.2 - 60)
```

- **Grade2-1:Learning3-2:**

$$
(M_{\text{文脈1}} + M_{\text{キー2}}) 
- (M_{\text{キー1}} + M_{\text{文脈2}})
$$

$$
(M_{\text{キー2}}-M_{\text{キー1}}) 
- (M_{\text{文脈2}} - M_{\text{文脈1}})
$$

```{r}
(49.8 - 40) - (45.2 - 19.8)
```

- ```emmeans()```関数で下位検定を行う場合も、どのコーディングをあてはめても結果は同じになります。

```{r}
#| include: false
#| eval: false

library(emmeans)

# サンプルデータの作成
set.seed(123)
df <- data.frame(
  class = rep(c("X", "Y"), each = 15),  # 2つのクラス
  group = rep(c("A", "B", "C"), each = 5, times = 2),  # 3つのグループ
  score = c(
    rnorm(5, mean = 5, sd = 1), rnorm(5, mean = 7, sd = 1), rnorm(5, mean = 9, sd = 1),  # class X
    rnorm(5, mean = 6, sd = 1), rnorm(5, mean = 6.5, sd = 1), rnorm(5, mean = 7, sd = 1)  # class Y
  )
)

df$class <- factor(df$class)
df$group <- factor(df$group)

# Treatment contrast (デフォルト)
contrasts(df$class) <- contr.treatment(2)
contrasts(df$group) <- contr.treatment(3)

# Sum contrast
contrasts(df$class) <- contr.sum(2)
contrasts(df$group) <- contr.sum(3)

# Repeated contrast (MASS パッケージが必要)
library(MASS)
contrasts(df$class) <- contr.sdif(2)
contrasts(df$group) <- contr.sdif(3)

model <- lm(score ~ class * group, data = df)

# emmeans の適用
contrast(
  emmeans(model, pairwise ~ class | group), 
  adjust = "bonferroni"
)
```


::: callout-important
- 2水準 × 3水準の交互作用でも係数の解釈は結構複雑になります。デザインはシンプルな方が解釈する側も分析をしやすいです。
:::

## 次週までの課題
### 課題内容

1. 小テストに向けて今回の内容を復習する。必ず手でコードを入力してRを実行する。

2. ```week8_data.csv```のデータを用いて質的変数 * 量的変数、質的変数 * 質的変数、もしくは量的変数 * 量的変数のいずれかの交互作用を含む回帰分析を行う。記述統計や図示、結果の報告を行うこと。

### 提出方法
-   Quartoファイルで作成し、HTMLファイルに変換しTACTへ提出（Quartoファイルも提出すること）

-   締め切りは今週の木曜日23:59まで

## 参考文献

- 📚南風原（2002）『心理統計学の基礎―統合的理解のために』有斐閣アルマ

- 📚馬場（2019）『RとStanではじめる　ベイズ統計モデリングによるデータ分析入門』講談社

- 🗣小島（2022）「外国語教育研究における（一般化）線形混合モデル：仮説に適したコーディング・モデリングを中心に」The 2021 Annual Conference on Vocabulary Acquisition